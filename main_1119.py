...
from draw_mask import draw_masks
...
class Ui_MainWindow(QWidget):
    def __init__(self):
        ...
        self.dic = {
            ...
            '008324a52c5910d338807fb785ebaa0a':{13:[],10:[101,108,128,142],31:[158,204],28:[163],23:[]},
            'wen':{33:[183],9:[91,115,136,145],29:[174],32:[218],31:[160]},
            '3people':{1:[0,115,145],31:[157,204],24:[],33:[182]},
    ...
    def setupUi(self):
        ...
        #open image button
        self.pushButton_openImage = QPushButton(QIcon("./images/open.png"),' Select Image')
        ...
        #save image button
        self.pushButton_saveImage = QPushButton(QIcon("./images/segment.png"),'  Segment')
        ...
        self.exit_button = QPushButton(QIcon("./images/exit.png"),'  Exit')
    ...
    def openImage(self): 
        ...
        img = self._scale_image(img, 1024)

        scale_radio = self.logo_size[0] / img.shape[0]
        dim = (int(img.shape[1] * scale_radio),self.logo_size[0])
    ...
    def refine_masks(self,masks,labels,imag):
        ...
        for m in range(masks.shape[-1]):
            mask_raw = cv2.resize(masks[:,:,m], (imag.shape[1],imag.shape[0]), interpolation=cv2.INTER_NEAREST)
    ...
    def getResult(self):
        ...
        img = img.resize((self.config.width,self.config.height), resample = Image.BILINEAR)
        img = F.to_tensor(img)
        ...
        if masks.shape[-1]>0:
            ...
            masks = self.refine_masks(masks[:,:,:best_idx], labels[:best_idx],imag)
    ...
    def add(self):
        ...
        for i in range(len(self.classes)):
            ...
            if self.classes[i] in set(self.dic[self.image_name.split('.')[0]].keys()):
                attr = self.dic[self.image_name.split('.')[0]][self.classes[i]]
                for j in range(len(attr)):
                    if j == 0:
                        temp = self.attributes[attr[j]]['name']
                        description += temp
                    else:
                        temp = self.attributes[attr[j]]['name']
                        description += ', ' + temp 
